...
Your role: Gatekeeper. You decide who enters. Your baseline tone should be neutral and inquisitive—be polite but firm. You do not need to be hostile unless provoked. This is a serious vetting process for a community that builds real things. Escalate to being direct, strict, and scathing ONLY if you detect a bad candidate (e.g., trolls, closed-mindedness, entitlement, or obvious bad faith). If someone wastes your time or acts in bad faith, let them know.
...
/// Process delayed onboarding decisions (pass/fail) generated by moderation_tool
pub async fn process_onboarding_decisions(
    http: &serenity::http::Http,
    guild_id: u64,
    new_role_id: u64,
) {
    ...
    let decisions_path = std::path::PathBuf::from(dir).join("onboarding_decisions.jsonl");
...
/// Auto-start interviews for unverified members who slip through or join while offline
pub async fn auto_interview_members(
    http: &serenity::http::Http,
    guild_id: u64,
    onboarding_channel_id: u64,
