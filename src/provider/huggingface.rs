...
impl Provider for HuggingFaceProvider {
    ...
    async fn list_models(&self) -> Result<Vec<ModelSummary>> {
        ...
        let url =
            "https://huggingface.co/api/models?filter=gguf&sort=downloads&direction=-1&limit=20";
        ...
        let resp = req
            .send()
            .await
            .context("Failed to query HuggingFace Hub")?;
        let body: Vec<serde_json::Value> =
            resp.json().await.context("Failed to parse HF response")?;
    ...
    async fn chat(
        ...
        let resp = req
            .send()
            .await
            .context("Failed to send chat to HuggingFace")?;
        if !resp.status().is_success() {
        ...
        let parsed: serde_json::Value = resp
            .json()
            .await
            .context("Failed to parse HF chat response")?;
        let content = parsed
            .get("choices")
    ...
    async fn chat_sync(
        ...
        let resp = req
            .send()
            .await
            .context("Failed to send sync chat to HuggingFace")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let error = resp.text().await.unwrap_or_default();
    ...
    async fn embed(&self, text: &str, model: &str) -> Result<Vec<f32>> {
        ...
        let resp = req
            .send()
            .await
            .context("Failed to send embedding to HuggingFace")?;
        if !resp.status().is_success() {
            bail!("HuggingFace embedding error {}", resp.status());
        }

        let parsed: serde_json::Value =
            resp.json().await.context("Failed to parse HF embedding")?;
    ...
    async fn health(&self) -> Result<ProviderStatus> {
            ...
            .await
        {
            ...
            Ok(resp) => Ok(ProviderStatus {
                ...
                latency_ms: Some(start.elapsed().as_millis() as u64),
                error: Some(format!(
                    "Auth check returned {} (may still work for public models)",
                    resp.status()
                )),
                models_loaded: Vec::new(),
            }),
            Err(e) => Ok(ProviderStatus {
                available: false,
                latency_ms: None,
                error: Some(format!("Cannot reach HuggingFace: {}", e)),
                models_loaded: Vec::new(),
            }),
        }
    }
...
mod tests {
    ...
    fn test_auth_header_with_token() {
        let provider = HuggingFaceProvider::new(&HuggingFaceConfig {
            api_token: "hf_test123".to_string(),
            model_id: String::new(),
            endpoint: "https://api-inference.huggingface.co".to_string(),
        });
        assert_eq!(
            provider.auth_header(),
            Some("Bearer hf_test123".to_string())
        );
    }
}
