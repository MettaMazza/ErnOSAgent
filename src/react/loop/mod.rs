    ...
    loop {
        ...
        if config.context_length > 0 {
            ...
            if usage > 0.85 {
                ...
                let estimate =
                    |m: &Message| -> usize { (m.content.len() / 3 + 1) + (m.images.len() * 4000) };
        ...
        let _ = event_tx
            .send(ReactEvent::NeuralSnapshot(hud_snapshot.clone()))
            .await;
        ...
        if let Some(ref summary) = output.thought_spiral {
            tracing::warn!(
                turn = turn,
                "🌀 Thought spiral detected — re-prompting to complete"
            );
        ...
        let has_other = output
            .tool_calls
            .iter()
            .any(|tc| !schema::is_loop_terminator(tc));
        ...
        if has_none && !has_reply && !output.response_text.trim().is_empty() {
            tracing::info!(
                turn = turn,
                "Model output bare text with no tools — auto-wrapping into reply_request"
            );
            let mut args = serde_json::Map::new();
            args.insert(
                "message".to_string(),
                serde_json::Value::String(output.response_text.clone()),
            );
        ...
        if has_other {
            ...
            if same_tool_count >= 3 {
                ...
                let attempted_tools: Vec<String> = output
                    .tool_calls
                    .iter()
                    .filter(|tc| !schema::is_loop_terminator(tc))
                    .map(|tc| {
            ...
            let executed_tools: Vec<String> = output
                .tool_calls
                .iter()
                .filter(|tc| !schema::is_loop_terminator(tc))
                .map(|tc| {
        ...
        if has_reply {
            ...
            if reply_text.is_empty() {
                tracing::warn!(
                    turn = turn,
                    "reply/refuse_request had empty text — retrying"
                );
                ...
                messages.push(Message {
                    ...
                    content: format!(
                        "```json\n{{\"name\": \"{}\", \"arguments\": {}}}\n```",
                        reply_call.name,
                        serde_json::to_string(&reply_call.arguments).unwrap_or_default()
                    ),
                    images: Vec::new(),
                });
            ...
            tracing::info!(
                ...
                "Entering handle_reply (observer)"
            );
            ...
            match handle_reply(
                ...
                turn,
                &mut audit_passes,
                &mut total_audit_rejections,
                &mut consecutive_audit_rejections,
                &event_tx,
                &mut learning_ctx,
            )
            .await
        ...
        if has_none && !has_reply {
            tracing::warn!(
                turn = turn,
                response_len = output.response_text.len(),
                "No tool calls and no reply_request — injecting error"
            );
...
fn format_hud_message(snap: &crate::interpretability::snapshot::NeuralSnapshot) -> String {
    ...
    let source = if snap.is_live {
        "LIVE SAE"
    } else {
        "SIMULATED"
    };

    let features: String = snap
        .top_features
        .iter()
        .take(6)
...
fn inject_empty_reply_error(messages: &mut Vec<Message>) {
    messages.push(Message {
        ...
        content: "[ERROR: reply_request was called with an empty message. \
                  You must provide meaningful content in the message field.]"
            .to_string(),
        images: Vec::new(),
    });
}
...
fn inject_no_reply_error(response_text: &str, messages: &mut Vec<Message>, turn: usize) {
    tracing::warn!(
        ...
        "Model failed to call reply_request — injecting rejection feedback"
    );
