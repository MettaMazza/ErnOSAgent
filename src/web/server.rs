...
pub async fn start(
    ...
    println!("  ║   http://localhost:{:<5}                 ║", addr.port());
    println!("  ║                                         ║");
    println!("  ║   Mobile relay: ws://localhost:{}/ws/relay  ║", addr.port());
...
fn build_router(state: SharedState) -> Router {
    Router::new()
        ...
        .route("/api/sessions/{id}/rename", post(routes::sessions::rename_session))
        .route("/api/sessions/{id}/export", get(routes::sessions::export_session))
        .route("/api/sessions/{id}/react", post(routes::sessions::react_to_message))
        // REST API — Status
        ...
        .route("/api/learning/train", post(routes::status::trigger_training))
        ...
        .route("/api/observer/toggle", post(routes::status::toggle_observer))
        // REST API — Steering
        .route("/api/steering", get(routes::steering::get_steering))
        .route("/api/steering/{name}/scale", post(routes::steering::set_steering_scale))
        .route("/api/steering/{name}/toggle", post(routes::steering::toggle_steering))
        .route("/api/neural", get(routes::steering::neural_snapshot))
        .route("/api/neural/features", get(routes::steering::list_steerable_features))
        .route("/api/neural/steer", post(routes::steering::steer_feature))
        .route("/api/neural/steer", delete(routes::steering::clear_feature_steering))
        ...
        .route("/api/platforms/{platform}", post(routes::platform::save_platform))
        ...
        .route("/api/memory/consolidate", post(routes::memory::consolidate_memory))
        ...
        .route("/api/lessons/{id}/reinforce", post(routes::memory::reinforce_lesson))
        .route("/api/lessons/{id}/weaken", post(routes::memory::weaken_lesson))
        .route("/api/scratchpad", get(routes::memory::read_scratchpad))
        .route("/api/scratchpad", post(routes::memory::write_scratchpad))
        // REST API — Reasoning
        .route("/api/reasoning/traces", get(routes::reasoning::recent_traces))
        .route("/api/reasoning/search", get(routes::reasoning::search_traces))
        .route("/api/reasoning/stats", get(routes::reasoning::trace_stats))
        // REST API — Checkpoints
        .route("/api/checkpoints", get(routes::checkpoints::list_checkpoints))
        .route("/api/checkpoints", post(routes::checkpoints::create_checkpoint))
        .route("/api/checkpoints/{id}/restore", post(routes::checkpoints::restore_checkpoint))
        .route("/api/checkpoints/{id}", delete(routes::checkpoints::delete_checkpoint))
        // REST API — Scheduler
        .route("/api/scheduler/jobs", get(routes::scheduler::list_jobs))
        .route("/api/scheduler/jobs", post(routes::scheduler::create_job))
        .route("/api/scheduler/jobs/{id}", axum::routing::put(routes::scheduler::update_job))
        .route("/api/scheduler/jobs/{id}", delete(routes::scheduler::delete_job))
